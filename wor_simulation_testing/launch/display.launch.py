...
def generate_launch_description():
    ...
    default_rviz_config_path = PathJoinSubstitution([wor_simulation_testing_path, 'rviz', 'urdf.rviz'])
    ...
    ld.add_action(DeclareLaunchArgument(name='model', default_value=str(default_model_path),
                                        description='Path to robot urdf file relative to wor_simulation_testing package'))

    ld.add_action(IncludeLaunchDescription(
        PathJoinSubstitution([FindPackageShare('urdf_launch'), 'launch', 'display.launch.py']),
        launch_arguments={
            'urdf_package': 'wor_simulation_testing',
